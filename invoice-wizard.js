...
async function runFindTextConstellationLock(){
  ...
  if(isTess){
    ...
    await applyFindTextPreviewFromBox({
      boxPx: boxesPx[0],
      tokenSource: lock.tokenSource || 'tesseract',
      page
    });
  } else {
    ...
    await applyFindTextPreviewFromBox({
      boxPx: boxesPx[0],
      tokenSource: lock.tokenSource || 'pdfjs',
      page
    });
  }
...
async function applyFindTextPreviewFromBox({ boxPx, tokenSource, page } = {}){
  const stepper = state.findTextLearning.stepper;
  stepper.source = tokenSource || stepper.source || null;
  stepper.tokenSource = tokenSource || stepper.tokenSource || stepper.source || null;
  stepper.page = page || boxPx?.page || state.pageNum || 1;
